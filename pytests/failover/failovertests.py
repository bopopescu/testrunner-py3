...
class FailoverTests(FailoverBaseTest):
    ...
    def common_test_body(self, failover_reason):
        ...
        if self.failoverMain:
            self.chosen = RebalanceHelper.pick_nodes(
                self.main, howmany=1, target_node=self.servers[0])
        else:
            self.chosen = RebalanceHelper.pick_nodes(
                self.main, howmany=self.num_failed_nodes)
    ...
    def run_mutation_operations(self):
        ...
        if "create" in self.doc_ops:
            mutation_ops_tasks += self._async_load_all_buckets(
                self.main, self.gen_create, "create", 0)
        if "update" in self.doc_ops:
            mutation_ops_tasks += self._async_load_all_buckets(
                self.main, self.gen_update, "update", 0)
        if "delete" in self.doc_ops:
            mutation_ops_tasks += self._async_load_all_buckets(
                self.main, self.gen_delete, "delete", 0)
        try:
    ...
    def run_mutation_operations_after_failover(self):
        ...
        if "create" in self.doc_ops:
            mutation_ops_tasks += self._async_load_all_buckets(
                self.main, self.afterfailover_gen_create, "create", 0)
        if "update" in self.doc_ops:
            mutation_ops_tasks += self._async_load_all_buckets(
                self.main, self.afterfailover_gen_update, "update", 0)
        if "delete" in self.doc_ops:
            mutation_ops_tasks += self._async_load_all_buckets(
                self.main, self.afterfailover_gen_delete, "delete", 0)
        try:
            for task in mutation_ops_tasks:
                task.result()
        except Exception as ex:
