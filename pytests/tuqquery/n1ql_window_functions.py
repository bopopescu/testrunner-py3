...
class WindowFunctionsTest(QueryTests):
    ...
    def test_from_select(self):
        ...
        for f_name in exceptional_functions:
            ...
            if f_name == 'NTH_VALUE':
                query = "from (select decimal_field as f1, "+self._construct_window_function_call(func)+" as f2, char_field as f3 from test_bucket group by char_field, decimal_field) a " \
    ...
    def test_from_select_let_where(self):
        ...
        for f_name in exceptional_functions:
            func = self.window_functions[f_name]
            if f_name == 'NTH_VALUE':
                query = "from (select decimal_field as f1, "+self._construct_window_function_call(func)+" as f2, char_field as f3 from test_bucket group by char_field, decimal_field) a let cond = [157331,165546] " \
                                                                                                 "where a.f2 in cond select a.f1 , a.f2, "+func['name']+"(a.f2, 3) over (partition by a.f2), a.f3"
            elif f_name == 'LAG' or f_name == 'LEAD' or f_name == 'NTILE':
                query = "from (select decimal_field as f1, "+self._construct_window_function_call(func)+" as f2, char_field as f3 from test_bucket group by char_field, decimal_field) a let cond = [157331,165546] " \
                                                                                                 "where a.f2 in cond select a.f1 , a.f2, "+func['name']+"(a.f2) over (partition by a.f2 order by decimal_field), a.f3"
            else:
                query = "from (select decimal_field as f1, "+self._construct_window_function_call(func)+" as f2, char_field as f3 from test_bucket group by char_field, decimal_field) a let cond = [157331,165546] " \
                                                                                                 "where a.f2 in cond select a.f1 , a.f2, "+func['name']+"() over (partition by a.f2 order by a.f2), a.f3"
            lambda1 = lambda x: self.assertTrue(x['q_res'][0]['status'] == 'success')
            test_dict["%d-default" % (count)] = {"indexes": self.indexes,
                                                 "pre_queries": [],
                                                 "queries": [query],
    ...
    '''
        Test idea - check syntax correctness of very basic usage of all sorts of window functions 
        with the use of PARTITION BY parameter as a condition in LETTING HAVING clause and window function parameter as a part of GROUP BY. 
        Returned data set is not analyzed.
    '''
    ...
    '''
        Test idea - check syntax correctness of very basic usage of all sorts of window functions 
        with the use of window function call result as a condition in ORDER BY clause plus all possible ORDER BY options (direction, limit, offset). 
        Returned data set is not analyzed.
    '''
    def test_order_by_window_result(self):
        ...
        for k, v in self.window_functions.items():
            for op in asc_desc:
                for lim in limit:
                    for off in offset:
                        query = "select "+self._construct_window_function_call(v)+" as summa from test_bucket order by summa "+op+lim+off
                        lambda1 = lambda x: self.assertTrue(x['q_res'][0]['status'] == 'success')
                        test_dict["%d-default" % (count)] = {"indexes": self.indexes,
                                                             "pre_queries": [],
                                                             "queries": [query],
    ...
    def test_tcpds_3(self):
        query = "SELECT 'web' AS channel, web.item, web.return_ratio, web.return_rank,web.currency_rank " \
                ...
                "(SELECT ws.char_field AS item, (Sum(COALESCE(ws.int_field, 0)) / (Sum(COALESCE(ws.decimal_field, 0)) )) AS return_ratio," \
                "(Sum(COALESCE(ws.int_field, 0))) / (Sum(COALESCE(ws.decimal_field, 0))) AS currency_ratio " \
                "FROM test_bucket ws GROUP BY ws.char_field) in_web) web WHERE ( web.return_rank <= 10 OR web.currency_rank <= 10 ) " \
                "UNION SELECT 'catalog' AS channel,catalog.item,catalog.return_ratio,catalog.return_rank,catalog.currency_rank " \
                "FROM (SELECT item,return_ratio,currency_ratio,Rank() OVER (ORDER BY return_ratio) AS return_rank," \
                ...
                "FROM test_bucket cs GROUP BY cs.char_field) in_cat) catalog WHERE " \
    ...
    def test_first_value(self):
        query = "select first_value(decimal_field) over (partition by char_field order by decimal_field) as fv, decimal_field as df, char_field as cf from test_bucket"
        result = self.run_cbq_query(query)

        test_dict = dict()
        for alpha in self.alphabet:
            query_min = "select decimal_field from test_bucket where char_field='{0}' order by decimal_field".format(alpha)
    ...
    def test_ratio_to_report(self):
        query = "select ratio_to_report(decimal_field) over (partition by char_field) as ratio_to_report, decimal_field, char_field from test_bucket"
        result = self.run_cbq_query(query)
        test_dict = {}
        for alpha in self.alphabet:
            test_query = "select sum(decimal_field) from test_bucket where char_field='{0}'".format(alpha)
    ...
    def test_last_value(self):
        query = "select last_value(decimal_field) over (partition by char_field order by decimal_field RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as last_value, " \
                "decimal_field, char_field from test_bucket"
        result = self.run_cbq_query(query)

        test_dict = {}
        for alpha in self.alphabet:
            query_max = "select decimal_field from test_bucket where char_field='{0}' order by decimal_field desc".format(alpha)
    ...
    def test_row_number(self):
        ...
        query = "select sq.df as decimal_field, sq.cf as char_field, sq.rn from (select row_number() over (partition by char_field order by decimal_field) as rn, " \
                "decimal_field as df, char_field as cf from test_bucket) sq where sq.rn={0}".format(check_number)
        result = self.run_cbq_query(query)

        test_dict = {}
        for alpha in self.alphabet:
            test_query = "select decimal_field from test_bucket where char_field='{0}' order by decimal_field".format(alpha)
            test_result = self.run_cbq_query(test_query)
    ...
    def test_array_agg(self):
        query = "select distinct(char_field), array_agg(decimal_field) over (partition by char_field) as agg from test_bucket"
        result = self.run_cbq_query(query)
        db_dict = {}
        for res in result['results']:
        ...
        test_dict = {}
        for alpha in self.alphabet:
            test_query = "select array_agg(decimal_field) from test_bucket where char_field='{0}'".format(alpha)
            test_result = self.run_cbq_query(test_query)
    ...
    '''
    def test_avg(self):
        query = "select distinct(char_field), avg(decimal_field) over (partition by char_field) as agg from test_bucket"
        result = self.run_cbq_query(query)
        db_dict = {}
        for res in result['results']:
            char_key = str(res['char_field'])
        ...
        test_dict = {}
        for alpha in self.alphabet:
            test_query = "select avg(decimal_field) from test_bucket where char_field='{0}'".format(alpha)
            test_result = self.run_cbq_query(test_query)
            test_dict[alpha] = test_result['results'][0]
    ...
    '''
    def test_count(self):
        query = "select distinct(char_field), count(decimal_field) over (partition by char_field) as agg from test_bucket"
        result = self.run_cbq_query(query)
        db_dict = {}
        for res in result['results']:
            char_key = str(res['char_field'])
        ...
        test_dict = {}
        for alpha in self.alphabet:
            test_query = "select count(decimal_field) from test_bucket where char_field='{0}'".format(alpha)
            test_result = self.run_cbq_query(test_query)
            test_dict[alpha] = test_result['results'][0]
    ...
    def test_countn(self):
        query = "select distinct(char_field), countn(decimal_field) over (partition by char_field) as agg from test_bucket"
        result = self.run_cbq_query(query)
        db_dict = {}
        for res in result['results']:
            char_key = str(res['char_field'])
        ...
        test_dict = {}
        for alpha in self.alphabet:
            test_query = "select countn(decimal_field) from test_bucket where char_field='{0}'".format(alpha)
            test_result = self.run_cbq_query(test_query)
            test_dict[alpha] = test_result['results'][0]
    ...
    '''
    def test_max(self):
        query = "select distinct(char_field), max(decimal_field) over (partition by char_field) as agg from test_bucket"
        result = self.run_cbq_query(query)
        db_dict = {}
        for res in result['results']:
            char_key = str(res['char_field'])
        ...
        test_dict = {}
        for alpha in self.alphabet:
            test_query = "select max(decimal_field) from test_bucket where char_field='{0}'".format(alpha)
            test_result = self.run_cbq_query(test_query)
            test_dict[alpha] = test_result['results'][0]
    ...
    '''
    def test_mean(self):
        query = "select distinct(char_field), mean(decimal_field) over (partition by char_field) as agg from test_bucket"
        result = self.run_cbq_query(query)
        db_dict = {}
        for res in result['results']:
            char_key = str(res['char_field'])
        ...
        test_dict = {}
        for alpha in self.alphabet:
            test_query = "select mean(decimal_field) from test_bucket where char_field='{0}'".format(alpha)
            test_result = self.run_cbq_query(test_query)
            test_dict[alpha] = test_result['results'][0]
    ...
    def test_median(self):
        query = "select distinct(char_field), median(decimal_field) over (partition by char_field) as agg from test_bucket"
        result = self.run_cbq_query(query)
        db_dict = {}
        for res in result['results']:
            char_key = str(res['char_field'])
        ...
        test_dict = {}
        for alpha in self.alphabet:
            test_query = "select median(decimal_field) from test_bucket where char_field='{0}'".format(alpha)
            test_result = self.run_cbq_query(test_query)
            test_dict[alpha] = test_result['results'][0]
    ...
    '''
    def test_min(self):
        query = "select distinct(char_field), min(decimal_field) over (partition by char_field) as agg from test_bucket"
        result = self.run_cbq_query(query)
        db_dict = {}
        for res in result['results']:
            char_key = str(res['char_field'])
        ...
        test_dict = {}
        for alpha in self.alphabet:
            test_query = "select min(decimal_field) from test_bucket where char_field='{0}'".format(alpha)
            test_result = self.run_cbq_query(test_query)
            test_dict[alpha] = test_result['results'][0]
    ...
    '''
    def test_sum(self):
        query = "select distinct(char_field), sum(decimal_field) over (partition by char_field) as agg from test_bucket"
        result = self.run_cbq_query(query)
        db_dict = {}
        for res in result['results']:
            char_key = str(res['char_field'])
    ...
    def test_stddev(self):
        query = "select distinct(char_field), stddev(decimal_field) over (partition by char_field) as agg from test_bucket"
        result = self.run_cbq_query(query)
        db_dict = {}
        for res in result['results']:
            char_key = str(res['char_field'])
        ...
        test_dict = {}
        for alpha in self.alphabet:
            test_query = "select stddev(decimal_field) from test_bucket where char_field='{0}'".format(alpha)
            test_result = self.run_cbq_query(test_query)
            test_dict[alpha] = test_result['results'][0]
    ...
    def test_stddev_samp(self):
        query = "select distinct(char_field), stddev_samp(decimal_field) over (partition by char_field) as agg from test_bucket"
        result = self.run_cbq_query(query)
        db_dict = {}
        for res in result['results']:
            char_key = str(res['char_field'])
        ...
        test_dict = {}
        for alpha in self.alphabet:
            test_query = "select stddev_samp(decimal_field) from test_bucket where char_field='{0}'".format(alpha)
            test_result = self.run_cbq_query(test_query)
            test_dict[alpha] = test_result['results'][0]
    ...
    def test_stddev_pop(self):
        query = "select distinct(char_field), stddev_pop(decimal_field) over (partition by char_field) as agg from test_bucket"
        result = self.run_cbq_query(query)
        db_dict = {}
        for res in result['results']:
            char_key = str(res['char_field'])
        ...
        test_dict = {}
        for alpha in self.alphabet:
            test_query = "select stddev_pop(decimal_field) from test_bucket where char_field='{0}'".format(alpha)
            test_result = self.run_cbq_query(test_query)
            test_dict[alpha] = test_result['results'][0]
    ...
    def test_variance(self):
        query = "select distinct(char_field), variance(decimal_field) over (partition by char_field) as agg from test_bucket"
        result = self.run_cbq_query(query)
        db_dict = {}
        for res in result['results']:
            char_key = str(res['char_field'])
        ...
        test_dict = {}
        for alpha in self.alphabet:
            test_query = "select variance(decimal_field) from test_bucket where char_field='{0}'".format(alpha)
            test_result = self.run_cbq_query(test_query)
            test_dict[alpha] = test_result['results'][0]
    ...
    def test_var_samp(self):
        query = "select distinct(char_field), var_samp(decimal_field) over (partition by char_field) as agg from test_bucket"
        result = self.run_cbq_query(query)
        db_dict = {}
        for res in result['results']:
            char_key = str(res['char_field'])
        ...
        test_dict = {}
        for alpha in self.alphabet:
            test_query = "select var_samp(decimal_field) from test_bucket where char_field='{0}'".format(alpha)
            test_result = self.run_cbq_query(test_query)
            test_dict[alpha] = test_result['results'][0]
    ...
    def test_var_pop(self):
        query = "select distinct(char_field), var_pop(decimal_field) over (partition by char_field) as agg from test_bucket"
        result = self.run_cbq_query(query)
        db_dict = {}
        for res in result['results']:
            char_key = str(res['char_field'])
        ...
        test_dict = {}
        for alpha in self.alphabet:
            test_query = "select var_pop(decimal_field) from test_bucket where char_field='{0}'".format(alpha)
            test_result = self.run_cbq_query(test_query)
            test_dict[alpha] = test_result['results'][0]
