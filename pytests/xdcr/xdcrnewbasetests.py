...
class CouchbaseCluster:
    ...
    def init_cluster(self, disabled_consistent_view=None):
        ...
        self.enable_diag_eval_on_non_local_hosts(self.__main_node)
    ...
    def enable_diag_eval_on_non_local_hosts(self, main):
        ...
        if output is not None:
            if "ok" not in output:
                self.__log.error("Error in enabling diag/eval on non-local hosts on {}".format(main.ip))
    ...
    def set_internal_setting(self, param, value):
        output, _ = RemoteMachineShellConnection(self.__main_node).execute_command_raw(
            "curl -X GET http://Administrator:password@localhost:9998/xdcr/internalSettings")
        if not '"' + param + '":' + value in output:
            RemoteMachineShellConnection(self.__main_node).execute_command_raw(
                "curl http://Administrator:password@localhost:9998/xdcr/internalSettings -X POST -d " +
                param + '=' + value)
        output, _ = RemoteMachineShellConnection(self.__main_node).execute_command_raw(
            "curl -X GET http://Administrator:password@localhost:9998/xdcr/internalSettings")
        self.__log.info("{0}:{1}".format(self.__main_node.ip, output))
    ...
    def verify_meta_data(self, kv_store=1, timeout=None, skip=[]):
        ...
        for bucket in self.__buckets:
            if bucket.name not in skip:
                data_map = {}
                gather_task = self.__clusterop.async_get_meta_data(self.__main_node, bucket, bucket.kvs[kv_store],
                                                        compression=self.sdk_compression)
                gather_task.result()
...
class XDCRNewBaseTest(unittest.TestCase):
    ...
    def verify_filtered_items(self, src_main, dest_main, replications, skip_index=False):
        for repl in replications:
            # Assuming src and dest bucket of the replication have the same name
            bucket = repl['source']
            if not skip_index:
                self._create_index(src_main, bucket)
                self._create_index(dest_main, bucket)
            if repl['filterExpression']:
                filter_exp = repl['filterExpression']
            else:
                self.fail("Replication created without any filter.")
            src_count = self._get_doc_count(src_main, filter_exp, bucket)
            dest_count = self._get_doc_count(dest_main, filter_exp, bucket)
            if src_count != dest_count:
                self.fail("Doc count {0} on {1} does not match "
                          "doc count {2} on {3} "
                          "after applying filter {4}"
                          .format(src_count, src_main.ip,
                                  dest_count, dest_main.ip,
                                  filter_exp))
